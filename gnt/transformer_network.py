...
class GNT(nn.Module):
    ...
    def forward(self, rgb_feat, deep_sem_feat, ray_diff, mask, pts, ray_d):
        ...
        sem_feat_2d = torch.stack(deep_sem_2d, dim=0).sum(dim=0).mean(dim=1)
        sem_feat_2d = self.semantic_adaptor(sem_feat_2d)
        return torch.cat([outputs, attn], dim=1), sem_feat_2d, sem_feat_3d

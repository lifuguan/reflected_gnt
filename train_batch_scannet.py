...
from train_scannet import log_view, init_distributed_mode
import wandb 
...
def train(args):
    ...
    while global_step < model.start_step + args.n_iters + 1:
        for train_data in train_loader:
            ...
            device = que_deep_semantics.device
            loss = 0
            # for i in range(args.batch_size):

            tmp_train_data = {
                "rgb": train_data["rgb"][0].unsqueeze(0), 
                "labels": train_data["labels"][0].unsqueeze(0),
                "camera": train_data["camera"][0].unsqueeze(0),
                "rgb_path": train_data["rgb_path"][0],
                "src_rgbs": train_data["src_rgbs"][0].unsqueeze(0),
                "src_cameras": train_data["src_cameras"][0].unsqueeze(0),
                "depth_range": train_data["depth_range"][0].unsqueeze(0),
            }
            # reference feature extractor
            ref_coarse_feats, ref_fine_feats, ref_deep_semantics = model.feature_net(tmp_train_data["src_rgbs"].squeeze(0).permute(0, 3, 1, 2).to(device))
            ref_deep_semantics = model.feature_fpn(ref_deep_semantics)
            # load training rays
            ray_sampler = RaySamplerSingleImage(tmp_train_data, device)
            N_rand = int(
                1.0 * args.N_rand * args.num_source_views / tmp_train_data["src_rgbs"][0].shape[0]
            )
            ...
            loss += render_loss['train/rgb-loss']
            ...
            loss += semantic_loss['train/semantic-loss'][0]
            ...
            scalars_to_log["train/semantic-loss"] = semantic_loss['train/semantic-loss'][0].item()
